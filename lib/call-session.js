...
Duration=${payload.duration} `
  ...
  async _onReinvite(dlg, req, res) {
    try {
      ...
      if (process.env.JAMBONES_CODEC_OFFER_WITH_ORDER) {
        const parsedSdp = sdpTransform.parse(req.body);

        // Modify audio media section to only include PCMU and telephone-event
        parsedSdp.media.forEach((media) => {
          if (media.type === 'audio') {
            // Set new RTP mappings for PCMU and telephone-event
            media.rtp = [
            ...
            // Set payloads
            media.payloads = '0 101';

            // Set fmtp for telephone-event
            media.fmtp = [
              { payload: 101, config: '0-16' }
            ];

            // Remove any existing rtcpFb
            delete media.rtcpFb;
          }
        });

        // Convert back to SDP string
        req.body = sdpTransform.write(parsedSdp);
        this.logger.debug({ modifiedSdp: req.body }, 'Modified SDP with PCMU and telephone-event only');
