        ...
        try {
          ...
          if (gws.length) {
            ...
            gws.forEach((o) => {
              ...
              } if (vc.sip_proxy && vc.sip_proxy.length > 0) {
                if (isValidDomainOrIP(vc.sip_proxy)) {
                  proxy = `${scheme}:${prefix}${prependPlus}${calledNumber}@${vc.sip_proxy};transport=${transport}`;
                  this.logger.debug({uri: u}, `using outbound proxy for this trunk: ${proxy}`);
                } else {
                  this.logger.info({voip_carrier_sid, sip_proxy: vc.sip_proxy},
                    'invalid sip_proxy found for requested carrier');
                }
              }
