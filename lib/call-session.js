  ...
  async _onInfo(dlg, req, res) {
    try {
      ...
      if (dlg.type === 'uas' && req.has('X-Reason')) {
        ...
        if (reason.startsWith('mute')) {
        ...
        } else if (reason.includes('InbandDtmf')) {
          const arr = /Signal=\s*([0-9#*])/.exec(req.body);
          if (!arr) {
            this.logger.info({body: req.body}, '_onInfo: invalid INFO InbandDtmf');
          ...
          if ('ok' !== response.result) {
            this.logger.info({response}, `rtpengine play InbandDtmf failed with ${JSON.stringify(response)}`);
            throw new Error('rtpengine failed: answer');
          }
        }
      }
