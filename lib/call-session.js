...
class CallSession extends Emitter {
  ...
  async connect() {
    const {rootSpan, ...locals} = this.req.locals;
    const childSpan = rootSpan.startChildSpan('outbound:connect', {});
    const teams = this.teams = this.req.locals.target === 'teams';
    const engine = this.srf.locals.getRtpEngine();
    if (!engine) {
      childSpan.endWithError('no rtp engines');
    ...
    try {
      ...
      debug(`connecting call: ${JSON.stringify(locals)}`);
      let headers = {
      ...
      childSpan.setAttributes(headers);
      if (this.req.locals.registration) {
        childSpan.setAttributes({callType: 'user'});
        ...
        if (this.req.has('X-Override-To')) {
          ...
          childSpan.setAttributes({'X-Override-To': dest});
        }
        uris = [destUri];
        ...
        childSpan.setAttributes({lcr: 'true'});
        debug('calling lcr');
        try {
      ...
      if (this.req.canceled) {
        childSpan.endWithError('abandoned');
          ...
          const {uas, uac} = await this.srf.createB2BUA(this.req, this.res, uri, {
            ...
            localSdpA: async(sdp, res) => {
              ...
              if ('ok' !== response.result) {
                ...
                childSpan.endWithError(`rtpengine failed: ${response['error-reason']}`);
      ...
      childSpan.endWithError('Error setting up outbound');
