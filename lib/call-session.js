  ...
  async connect() {
    ...
    try {
      ...
      if (this.req.locals.registration) {
        ...
        let forwardUri = this.req.uri;
        if (forwardUri.startsWith('sips:')) {
          const parsed = parseUri(forwardUri);
          if (!parsed.params?.transport) {
            parsed.params = {...(parsed.params || {}), transport: 'tls'};
            forwardUri = stringifyUri(parsed);
          }
          this.logger.info({uri: forwardUri}, 'using SRTP/TLS for forwarded sips: call');
          this.rtpEngineOpts = makeRtpEngineOpts(this.req, false, true, false, false);
          encryptedMedia = true;
        }
        uris = [{
          private_network: await isPrivateVoipNetwork(forwardUri),
          uri: forwardUri
        }];
