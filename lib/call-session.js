...
class CallSession extends Emitter {
  constructor(logger, req, res) {
    ...
    const {performLcr, lookupCarrierBySid, lookupSipGatewaysByCarrier, lookupCarrierByAccountLcr} = this.srf.locals.dbHelpers;
      ...
      if (this.req.locals.registration) {
        ...
        try {
          ...
          if (!voip_carrier_sid && account_sid) {
            debug('calling lcr');
            voip_carrier_sid = this.lookupCarrierByAccountLcr(account_sid, this.req.calledNumber);
          }
          if (voip_carrier_sid) {
