...
class CallSession extends Emitter {
  ...
  async connect() {
    ...
    const destIsOverTls = (() => {
      if (!this.req.uri) return false;
      const parsed = parseUri(this.req.uri);
      if (!parsed) return false;
      if (parsed.scheme?.toLowerCase() === 'sips') return true;
      const transport = parsed.params?.transport;
      return typeof transport === 'string' && transport.toLowerCase() === 'tls';
    })();
    const srtpProfile = teams ? 'teams' : (destIsOverTls ? 'sdes' : null);
    this.rtpEngineOpts = makeRtpEngineOpts(
      this.req, false,
      destIsOverTls || this.useWss || teams,
      false,
      srtpProfile
    );
          ...
          if (gws.length) {
            ...
            gws.forEach((o) => {
              ...
              if (o.protocol === 'tls/srtp') {
                ...
                this.rtpEngineOpts = makeRtpEngineOpts(this.req, false, true, o.pad_crypto, 'teams', o.remove_ice, o.dtls_off);
                encryptedMedia = true;
              }
            });
