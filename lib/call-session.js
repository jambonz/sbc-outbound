...
class CallSession extends Emitter {
  ...
  async connect() {
    ...
    const srtpHeader = this.req.get('X-Jambonz-SRTP');
    const useSdes = srtpHeader === 'sdes';
    this.rtpEngineOpts = makeRtpEngineOpts(this.req, false, useSdes || this.useWss || teams, {
      profile: useSdes ? 'sdes' : (teams ? 'teams' : 'default')
    });
    this.rtpEngineResource = {destroy: this.del.bind(null, this.rtpEngineOpts.common)};
    let proxy, uris = [];
    const mapGateways = new Map();
    let encryptedMedia = useSdes;
        ...
        try {
          ...
          if (gws.length) {
            ...
            gws.forEach((o) => {
              ...
              if (o.protocol === 'tls/srtp' && !srtpHeader) {
                ...
                this.rtpEngineOpts = makeRtpEngineOpts(this.req, false, true, {
                  profile: 'teams',
                  padCrypto: o.pad_crypto,
                  removeIce: o.remove_ice,
                  dtlsOff: o.dtls_off
                });
                encryptedMedia = true;
              }
