...
Duration=${payload.duration} `
  ...
  async _onReinvite(dlg, req, res) {
    try {
      ...
      offerMedia = { ...offerMedia, ...(acceptCodecs && this.req.locals.cdr.direction ==="outbound" && { codec: { mask: acceptCodecs, transcode: 'pcmu,pcma' } })};
      answerMedia = { ...answerMedia, ...(process.env.JAMBONES_CODEC_OFFER_WITH_ORDER && this.req.locals.cdr.direction ==="outbound" && { codec: {offer:
