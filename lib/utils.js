...
function makeRtpEngineOpts(req, srcIsUsingSrtp, dstIsUsingSrtp, padCrypto, srtpProfile, remove_ice = false, dtls_off = false) {
  ...
  const profileName = srtpCopy[srtpProfile] ? srtpProfile : 'default';
  const srtpOpts = srtpCopy[profileName];
  const isTeams = profileName === 'teams';

  if (padCrypto) {
    srtpOpts.flags.push('SDES-pad');
  }
