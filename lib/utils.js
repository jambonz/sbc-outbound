...
function makeRtpEngineOpts(req, srcIsUsingSrtp, dstIsUsingSrtp, opts = {}) {
  const {
    padCrypto = false,
    profile = 'default',
    removeIce = false,
  ...
  if (padCrypto) {
    ...
    if (srtpCopy['sdes']) srtpCopy['sdes'].flags.push('SDES-pad');
  }

  const srtpOpts = srtpCopy[profile] || srtpCopy['default'];
