...
module.exports = (srf, logger, opts) => {
  ...
  const createRootSpan = (req, res, next) => {
    ...
    const newTraceId = req.get('X-Trace-ID');
    const rootSpan = new RootSpan('sbc-incoming-call', newTraceId, {}, tracer, logger);
  ...
  const checkLimits = async(req, res, next) => {
    const {logger, account_sid, service_provider_sid, application_sid, rootSpan} = req.locals;
    ...
    const childSpan = rootSpan.startChildSpan('outbound:checkLimits');
    if (!process.env.JAMBONES_HOSTING && !trackingOn) {
      childSpan.setAttributes({trackingOn: 'false'});
      childSpan.end();
    ...
    try {
      ...
      if (limit) {
        ...
        if (calls > limit_sessions) {
          childSpan.setAttributes({calls, limit_sessions});
          childSpan.endWithError('call limit exceeded');
        ...
        if (process.env.JAMBONES_TRACK_ACCOUNT_CALLS && account_limit > 0 && calls > account_limit) {
          childSpan.setAttributes({calls, account_limit});
          childSpan.endWithError('account limit exceeded');
        ...
        if (process.env.JAMBONES_TRACK_SP_CALLS && sp_limit > 0 && callsSP > sp_limit) {
          childSpan.setAttributes({callsSP, sp_limit});
          childSpan.endWithError('service provider limit exceeded');
      ...
      childSpan.endWithError('error checking limits');
